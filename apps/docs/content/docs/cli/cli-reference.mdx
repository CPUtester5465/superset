...
<Command
	...
	options={[
		{ flag: "--workspace <id>", description: "Workspace UUID (omit to list every workspace on the host)." },
		{ flag: "--host <id>", description: "Host to list on (default: this machine)." },
	]}
	output={`{
  sessions: Array<TerminalSession>;
}`}
>
List live terminal sessions — the whole host by default, or one workspace with
`--workspace`. Presence in the list means the PTY exists, not that an agent
inside it is working or idle.
</Command>
